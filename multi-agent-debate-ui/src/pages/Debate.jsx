...
export default function Debate() {
  ...
  const generateDebatePrompt = (side, previousResponses) => {
    ...
    const context = previousResponses.length > 0 
      ? `\nDebate history:\n${allResponses}\n\nNow it's your turn to ${side === 'for' ? 'support' : 'oppose'} the topic. Address the previous arguments and advance the debate.`
      : '';
...
Guidelines:
- Directly address and respond to the previous arguments
- Identify key points in the opponent's argument and address them specifically
- Present new arguments or evidence to support your position
...
Your response:`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update with both initial responses and start the continuous debate
      setDebate({
        forResponses: [initialResponse],
        againstResponses: [againstResponse]
      });

      // Start the continuous debate with the Blue agent's next response
      setIsLoading(false);
      startContinuousDebate('for');
    } catch (err) {
  ...
  const startContinuousDebate = (startingSide) => {
    if (!isDebating) return;
    setTimeout(() => continueDebate(startingSide), 500);
  };
  ...
  const continueDebate = async (currentSide) => {
    ...
    try {
      ...
      // Get all previous responses in chronological order
      const allResponses = [];
      const forResponses = debate?.forResponses || [];
      const againstResponses = debate?.againstResponses || [];
      const maxLength = Math.max(forResponses.length, againstResponses.length);

      for (let i = 0; i < maxLength; i++) {
        if (forResponses[i]) {
          allResponses.push({
            role: 'for',
            content: forResponses[i]
          });
        }
        if (againstResponses[i]) {
          allResponses.push({
            role: 'against',
            content: againstResponses[i]
          });
        }
      }

      console.log(`Generating response for ${currentSide} agent (Round ${maxLength + 1})`);
      console.log('Previous responses:', allResponses);

      // Generate the next response
      const response = await generateResponse(
        generateDebatePrompt(currentSide, allResponses.map(r => r.content)),
        currentSide,
        provider
      );

      // Update the debate state with the new response
      ...
      // Continue the debate with the next agent if still active
      if (isDebating) {
        setIsLoading(false);
        const nextSide = currentSide === 'for' ? 'against' : 'for';
        startContinuousDebate(nextSide);
      }
    } catch (err) {
